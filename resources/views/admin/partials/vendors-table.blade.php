@if ($vendors->isEmpty())
    <div class="text-center font-semibold text-lg">No Vendors Found</div>
@else
                ...
                @foreach ($vendors as $vendor)
                    <tr
                        ...
                        <td class="flex items-center px-6 py-4">
                            <span class="mr-4 cursor-pointer edit_vendor_btn" data-id="{{ $vendor->id }}"
                            ...
                            <span class="delete_vendor_btn cursor-pointer" data-id="{{ $vendor->id }}">
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24"
...
<script>
    $(document).ready(function() {
        ...
        $('#editVendor').submit(function(event) {
        ...
        $('.delete_vendor_btn').on('click', function() {
            ...
            if (confirm('Are you sure you want to delete this vendor?')) {
                $.ajax({
                    url: `{{ route('vendors.destroy', ':id') }}`.replace(':id', vendorId),
                    type: 'POST',
                    data: {
                        _method: 'DELETE',
                        _token: '{{ csrf_token() }}'
